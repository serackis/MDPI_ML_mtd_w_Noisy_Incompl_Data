...
%	A single paragraph of about 200 words maximum. For research articles, abstracts should give a pertinent overview of the work. We strongly encourage authors to use the following style of structured abstracts, but without headings: (1) Background: Place the question addressed in a broad context and highlight the purpose of the study; (2) Methods: Describe briefly the main methods or treatments applied; (3) Results: Summarize the article's main findings; and (4) Conclusion: Indicate the main conclusions or interpretations. The abstract should be an objective representation of the article, it must not contain results which are not presented and substantiated in the main text and should not exaggerate the main conclusions.
	In this article we present a data-driven approach for a single speech 
	% AB: Are we restricted to speech or could it be any audio source signal?
    % SS: I have been using noise signal, and the results were much better than with speech. I have collected the noise dataset along with the speech dataset in the same room at the same time. But the speech is more practically applicable?
	source localization within an acoustic enclosure. 
	Our method %AB: I would begin with the big idea we are following before describing the details of the method: We leverage unlabelled data points by learning the intrinsic structure of high-dimensional feature vectors and incorporating this knowledge into the training of the NN. This structure is represented by a graph which regularizes the NN loss function.
	consists of high-dimensional acoustic feature extraction, selection based on a fitness criterion, feature manifold learning and low-dimensional embedding, graph dataset construction and an application of a graph-regularized neural network (GRNN) to learn the mapping between the embedded feature coordinates and the Cartesian coordinates of the sound source. Our method relies on the assumption of the feature space spatial smoothness. We present the experimental results of the speech source localization in real acoustic enclosures using two compact circular microphone arrays. We compare the performance of the GRNN for single speech sound source localization to the performance of two baseline algorithms. 
}
...
\emph{Do "camera aiming" %AB: I would call it camera steering, but yes, this can be considered as a use case. In addition to that many speech enhancement methods rely on knowing the position of the sound source, e.g., many beamforming approaches.
is one of the classical tasks that has such name?} Camera aiming can be inaccurate due to the parallax error \emph{citation is needed here} if the camera and the microphone array are not rotating on the same axis.%AB: Is this relevant in this context?
To accurately steer the camera towards a certain point in space in such setup, a complete set of coordinates, either Cartesian or polar,%AB:  relevant?
...
There are plenty of source DOA estimation methods utilizing compact arrays \cite{bruttiComparisonDifferentSound2008,wengThreedimensionalSoundLocalization2001,awad-allaTwostageApproachPassive2020}, %AB: For DOA estimation, we should cite also the classic methods: MUSIC, ESPRIT, GCC-PHAT, SRP-PHAT etc.
but only a few methods are offered for 2D or 3D source localization that can estimate both DoA and the distance or the %AB:  We did that for a concentrated array at EUSIPCO 2019. There are probably more learning-based methods. For multiple arrays, there are actually many such methods, see, e.g., our works or the works of Mouchtaris, Gannot, Cobos, etc.
Cartesian coordinates of the sound source, involving beamforming and particle filtering \cite{valinRobust3DLocalization2006} averaged directivity patterns of blind source separation systems \cite{4959563}. %AB: At least this method does only estimate DOA, but not the range

It might be desired to use compact microphone arrays that have as few microphones as possible. By ``compact'' we here imply that the dimensions of the microphone array are much smaller than the dimensions of the source localization space. %AB: This will be true for any array ;-) Maybe much smaller than the source-microphone distance? 
%SS: Yes! I had in mind that there were examples where microphones are distributed around the perimeter of the room and thus the 'array' is of comparable size with the room
...
When invoking sound source localization algorithms, usually simplifying free-field and far-field assumptions are made which aren't always accurate, especially inside acoustic enclosures, in which the diffuse field is observed due to acoustic reflections. %AB: due to multipath propagation (?)
%SS: probably more so than the diffuse field (is multipath propagation connected to diffuse field? does one cause another or not?)
...
%AB: I am not sure what you want to say here?
%SS:  I tried to say that the actual situation might be analyzed using a combination of assumptions about free and diffuse field, and that the data-driven approach might perform better when the sound field in the enclosure can not be analyzed using a single simplified model or assumption
...
%AB: Maybe merge both sentences or does they have different meanings?

There are several learning-based source localization approaches, based on either semi-supervised \cite{laufer-goldshteinSemiSupervisedSoundSource2016} or supervised \cite{heDeepNeuralNetworks2018b,heAdaptationMultipleSound2019,adavanneDirectionArrivalEstimation2017} learning paradigms. In both of these approaches % AB: classes of approaches (?)
to work, a set of acoustic features from known sound source positions (the labeled dataset) is needed. \todo{cite A. Brendel and colleagues}
...
Considering our setting, it is relatively easy to collect a large amount of acoustic features without labels,%AB: redundant?
and it is very tedious to provide labels (in our case -- the coordinates of the sound source) for such data. Such a learning-based strategy is promising as users spend most of their time in the same environments (living room, office, etc.), where such training data can be obtained and used.

%\subsection*{Motivation }
%AB: What is missing for me is the description of the main contributions: What are we doing in this paper which has not been done before? Why do we think that this is worth reading? -> Powerful NN model, using structure of the features, outperforming state-of-the-art methods etc.

In this article we present a method to localize a sound source in two dimensions using a two compact microphone arrays. %AB:  this should be merged with the sentence about compact arrays from above

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Materials and Methods}%AB: I would recommend to rename this to highlight the approach we are following: "Semi-supervised DNN-based sound source localization"

\noindent\makebox[\linewidth]{\rule{\paperwidth}{0.4pt}}

We assume that a sound source $ \src{} $ produces a signal $ \signal_{\src}(t) $.

We assume a set of $ \numel_{\arr} $ microphone arrays, each equipped with two $ \numel_{\mic} $ microphones.
Considering an acoustic enclosure in which a multipath propagation of the acoustic waves occurs, the signal $ \signal_{\arr,\mic}(t) $ received at a microphone $ \mic $ that belongs to array $ \arr $ can be described using a convolutive mixing model:
\begin{equation} \label{eq:signal_model}
    \signal_{\arr,\mic}(t) = 
    \signal_{\src}(t) \ast \IR_{\src,\arr,\mic} + \noise_{\arr,\mic}(t), 
    \quad M \in \numel_{\arr}, \quad m \in \numel_{\mic}.
\end{equation}



\paragraph{SRP-PHAT feature}

Array signal processing techniques rely on the ability to focus on signals originating from a particular
location or direction in space. Beamforming, in the
conventional sense, can be defined by a filter-and-sum process, which applies some temporal filters to the
microphone signals before summing them to produce a single, focused signal. If the location of the source is not known, then
a beamformer can be used to scan, or steer, over a predefined spatial region by adjusting its steering delays
(and possibly its filters). The output of a beamformer, when used in this way, is known as the \emph{steered response}.
The signal model, expressed in \eqref{eq:signal_model} allows $ \noise_{\arr,\mic}(t) $ to include reverberation as well as background noise. Since reverberation consists of multipath versions of the source signal $ \signal_{\src} $, $ \noise_{\arr,\mic} $ can be highly correlated with the $ \signal_{\src} $. Hence, the delay-and-sum beamforming is not always effective at suppressing this noise. Other adaptive beamforming methods
...
beamforming.
\cite{dibiaseHighaccuracyLowlatencyTechnique2000}

The steered response is obtained by sweeping the focus of the beamformer. When the focus corresponds to
the location of a sound source, the power of the steered response reaches a maximum, although a variety of
circumstances may causes it to peak when focused on other locations as well.
The steered response power (\SRP) can be expressed as the output power of a filter-and-sum beamformer and is defined as follows:
\begin{equation}\label{eq:SRP}
P(\Delta_1,\dots,\Delta_M) \equiv 
\int_{-\inf}^{\inf} 
Y      (\omega,\Delta_1,\dots,\Delta_M) 
Y\prime(\omega,\Delta_1,\dots,\Delta_M) \dif{\omega},
\end{equation}
where $ Y(\omega,\Delta_1,\dots,\Delta_M) $ is the output of the filter-and-sum beamformer and the $ Y\prime(\omega,\Delta_1,\dots,\Delta_M) $ is its complex conjugate.
\cite{dibiaseHighaccuracyLowlatencyTechnique2000}
...
srp_cost = np.zeros(self.grid.n_points) # this is the output vector

# apply PHAT weighting
# X is the Set of signals in the frequency (RFFT) domain for current frame.
absX = np.abs(X)          
absX[absX < tol] = tol #threshold at tol, tol is set as 1e-14
pX = X / absX #PHAT?

CC = []
for k in self.freq_bins: # integrate through all the frequencies
...
\emph{Now talk about ISOMAP. Few equations, 3-4 algorithmic steps.}
...
\emph{Then introduce Loss function.   norms instead of brackets – check code.}
...
Then discuss both components: supervised and unsupervised. Hui et al. neural structured learning.

The supervised loss component in the loss function provides a means for the neural network to learn the relation between the labeled input features and the physical quantities

\noindent\makebox[\linewidth]{\rule{\paperwidth}{0.4pt}}

Then talk about topology of the neural network.

\noindent\makebox[\linewidth]{\rule{\paperwidth}{0.4pt}}

Talk about ANN architecture – trial and error – actually it was found using hyperparameter optimization.

\noindent\makebox[\linewidth]{\rule{\paperwidth}{0.4pt}}

Found from randomly training 100 (?) archs. 5 best. There should be justification.
...
\ann{}-based sound source localization can be performed using various acoustic features that depend on the position of the sound source, either relative to the microphone array(s) or the acoustic enclosure.%AB:  not entirely clear

It can be assumed, that acoustic features are spatially smooth, that is, features obtained at spatially close source positions are also close in feature space. The relative distance or affinity of the features can be simply estimated by calculating the Euclidean distance between feature vectors. %AB: Well, this depends on the distance: For small distances yes, for large distances this will not be accurate. Hence, we are using manifold-driven distances
%SS: shall I add that the smoothness is local?

It can be assumed, that high-dimensional acoustic features %AB:  I would start this section with a description of the feature, argue that it is high-dimensional but influenced by only a few parameters and that it can be hence represented by a low-dim manifold
lie on a low-dimensional manifold, embedded in a high-dimensional feature space. 
Since the acoustic features are only dependent on the coordinates of the sound source %AB: I would not say that: If you take SRP-PHAT, the resulting energy distribution will be also dependent on, e.g., the reverberation time or the DRR.
%SS: I had in mind a static acoustic scene, in that case, the reverberation time is constant? The DDR is dependent on the reverberation time (which is assumed to be static) and the coordinates of the source and the array? Or are there more parameters?
, it is expected that the manifold would represent the spatial relations between the nearby acoustic features. 
...
When \ann{}s are utilized to obtain the sound source position via regression using acoustic features, it might be expected that the predictions of an \ann{} would %AB:  I would not say that it is expected but desired?
also exhibit spatial smoothness. A feature manifold could be used during the training of the \ann{} to ensure that the \ann{} learns the relation between the acoustic features and the source positions while also retaining the spatial smoothness. %AB:  I would highlight that this is the main idea and contribution of this paper
...
\subsection{Acoustic Features} %AB: I suggest to not use subsubsections as it costs a lot of space and the content of these subsubsections are mostly only a few sentences
We have considered several types of acoustic features, that are discussed further.

\subsubsection{Time Difference of Arrival} %AB: Are these features also used in the experiments or did you just test them?
